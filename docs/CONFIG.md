...
| `auto_allow_if_sandboxed` | bool，`false` | 仅在沙箱提供**文件写边界**（Linux bwrap write）时 execute 免确认。平台感知：Windows（Job 只限资源不隔离写盘）、macOS、Linux 无 bwrap、Linux `job` 档——这些"无文件写边界"的组合下自动失效（execute 恢复逐条确认，并打印原因）。黑名单始终生效 |
