...
Harness 维护三道门，读工具调用历史和 todo 状态：
...
### 3.6 工具错误韧性（_invoke_tool）

`tool.run(**args)` 被 `_invoke_tool` 包裹：任何异常（TypeError/ValueError/...）都变成结构化 tool result 回灌给模型，**不中断 turn**。

- 模型给 bash 传了 `path`（bash 只认 `cwd`）→ 返回 `Error: tool 'bash' rejected the arguments...`，模型读到后自己改参数重试
- 设计原则：**工具调用层面的错误不是错误，是信号**。只有底层 LLM API 错误（auth/网络/限流，从 run_step 抛出）才是致命的
