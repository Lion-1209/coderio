...
def test_load_keeps_all_messages_including_legacy_summary(tmp_path):
    """2026-09-20 (change plan Phase 3): the context_summary truncation
    pipeline was deleted — NOTHING produced those messages since the
    deepagents migration (compaction is the engine's own), so the pipeline
    was dead code. Legacy sessions on disk must still load, in FULL: the
    summary message itself is kept as a normal system message (the model
    sees it via _build_history_messages) and the messages it once
    superseded are no longer silently dropped. This test guards that the
    loader does not crash on, or lose, legacy files."""
    s = Session.create(tmp_path, {"meta": "test"})
    ...
    # Compaction happened here (legacy kind, no producer anymore):
    ...
    # Everything loads — no silent truncation.
    assert "old question 1" in contents
    assert "old answer 1" in contents
    assert any("explore" in c for c in contents if isinstance(c, str))
    # The legacy summary itself survives as a system message.
...
def test_load_no_summary_returns_all_messages(tmp_path):
    """A session without any context_summary is loaded unchanged."""
    s = Session.create(tmp_path, {"meta": "test"})
