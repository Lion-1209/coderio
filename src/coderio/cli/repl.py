...
def _sandbox_boundary(sandbox_mode: str) -> tuple[str, str | None]:
    """What boundary does the configured sandbox actually provide HERE?

    Returns (kind, gap_reason) where kind is:
      - "filesystem" — a real write boundary (Linux bwrap write mode)
      - "resource"   — process/resource limits only (Windows Job Object)
      - "none"       — the sandbox cannot deliver on this platform

    Third-party audit P1-11 (2026-09-04) + gpt5.6-sol review (2026-09-05):
    auto-allow is gated on "filesystem" — a Job Object bounds process count
    but NOT file writes, so auto-approving execute behind it was "zero
    isolation + zero confirmation". The rule is now one sentence: auto-allow
    only when a real write boundary exists.
    """
    if sandbox_mode == "off":
        return "none", None
    ...
    if sys.platform == "darwin":
        return "none", "macOS 暂无 OS 级沙箱（bwrap 仅支持 Linux）"
    # Linux
    if sandbox_mode == "write":
        try:
            ...
            return "none", "Linux 未安装 bubblewrap，write 沙箱将降级为直接执行"
        except ImportError:
            return "none", "linux_sandbox 模块不可用，write 沙箱将降级为直接执行"
    return "none", "Linux 下 job 档暂无实现（等价直接执行）"
...
def build_gate(cfg: Config, console=None, tui=None):
    ...
    # auto_allow_execute is gated on "filesystem" — the ONLY boundary kind
    # that justifies skipping the confirm prompt (audit P1-11 + gpt5.6-sol
    # review: a Job Object caps process count but NOT file writes, so
    # "resource" is zero-write-isolation and must not auto-approve).
    sandbox_mode = cfg.tools.sandbox_mode
    boundary_kind, boundary_gap = _sandbox_boundary(sandbox_mode)
    auto_exec = boundary_kind == "filesystem" and cfg.tools.auto_allow_if_sandboxed
    warn = None
    if sandbox_mode != "off" and cfg.tools.auto_allow_if_sandboxed and not auto_exec:
        gap = boundary_gap or "当前平台沙箱不提供文件写边界"
        warn = f"⚠ [sandbox] {gap}。auto_allow_if_sandboxed 不生效：execute 将逐条确认（黑名单仍生效）。"
    if warn:
